...
void update(ll nd, ll n_l, ll n_r, ll l, ll r, ll v){
	...
	tr[nd].mini = min(tr[2*nd].mini+tr[nd].prop,tr[2*nd+1].mini+tr[nd].prop);
}

ll query(ll nd, ll n_l, ll n_r, ll l, ll r, ll carry = 0){
	if(n_l >= l && n_r <= r){
		return (tr[nd].mini + carry);
	}
	if(n_l > r || n_r < l) return LLONG_MAX;
	ll mid = (n_l + n_r) / 2;
	return min(query(2*nd,n_l,mid,l,r,carry+tr[nd].prop),query(2*nd+1,mid+1,n_r,l,r,carry+tr[nd].prop));
}
